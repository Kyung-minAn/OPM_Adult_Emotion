...
for blockNum = 1:nBlocks
    ...
    for trialNum = 1:nTrialsPerBlock
        ...
        % Reset TTL to zero
        [buttonNum, pressTime] = waitForButtonPress(timeOutSeconds);
        
        % buttonNumSav(blockNum) = buttonNum;
                
        if (pressTime < maxScheduleFrames / fs)
            pause( (maxScheduleFrames / fs) - pressTime)
        end
        ...
        % Save timing data
        audioLength = (maxScheduleFrames / fs)-audioStart;
        trialData.audioStartTime = audioStart;
        trialData.audioEndTime = (maxScheduleFrames / fs);
        trialData.audioDuration = audioLength;
        trialData.trialEndTime = endTrlPTB;
        trialData.buttonNum = buttonNum;
        trialData.reactionTime = pressTime;
        
        %storecorrectresp
        try
            correctResp = (buttonNum==emotionType);
        catch
            correctResp = 2;
        end
        
        trialData.correctResp = correctResp;
...
fileName = sprintf('%s_PassiveEmoVoiceButton_%s.mat', participant, date_str);
...
excelPath = fullfile(saveDir, sprintf('%s_PassiveEmoVoiceButton_%s_summary.xlsx', ...
...
function [buttonNum, pressTime] = waitForButtonPress(timeOutSeconds)
% Returns:
%   buttonNum: 1 (Red), 2 (Green), 3 (Blue), 0 (no press/timeout)
%   pressTime: Time in seconds from function start, -1 if no press/timeout
%
% Example: [button, time] = waitForButtonPress(5)

% If timeOutSeconds is not provided, default to 5 seconds
if nargin < 1
    timeOutSeconds = 5;
end
...
try
    ...
    while ~buttonPressed && (GetSecs() - startTime) < timeOutSeconds
        ...
        if status.newLogFrames > 0
            ...
            elseif buttonCode == 64516  % Green button
                fprintf('Green button pressed\n');
                buttonNum = 2;
                pressTime = currentTime - startTime;
                % Different TTL pulse
                Datapixx('SetDoutValues', bin2dec('000000000000000000100000'));
                Datapixx('RegWrRd');
                WaitSecs(0.001);
                Datapixx('SetDoutValues', 0);
                Datapixx('RegWrRd');
            ...
            elseif buttonCode == 64520  % Blue button
                fprintf('Blue button pressed\n');
                buttonNum = 3;
                pressTime = currentTime - startTime;
                % Different TTL pulse
                Datapixx('SetDoutValues', bin2dec('000000000000000001000000'));
                Datapixx('RegWrRd');
                WaitSecs(0.001);
                Datapixx('SetDoutValues', 0);
                Datapixx('RegWrRd');
